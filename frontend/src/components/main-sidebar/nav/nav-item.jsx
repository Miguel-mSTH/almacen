    ...
    return (
      ...
      >
        <Link
          ...
          key={nav.uuid}
          menu-uuid={nav.uuid}
          onClick={(e, childStatus, uuid) =>
            this.handleToggle(
              e,
              (childStatus = nav.children ? 1 : 0),
              (uuid = nav.uuid)
            )
          }
        >
          ...
          {(() => {
            if (nav.children) {
              ...
              return <span className="toggle-none">{nav.name}</span>;
            } else {
              return <React.Fragment>{nav.name}</React.Fragment>;
            }
          })()}
        </Link>
